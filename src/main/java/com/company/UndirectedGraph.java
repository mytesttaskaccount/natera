...
public final class UndirectedGraph<Vertex> implements IGraph<Vertex> {
    private static class Edge<Vertex> implements IEdge<Vertex> {
        private Vertex to;

        private Edge(Vertex to) {
            if (to == null) {
    ...
    Map<Vertex, Set<IEdge<Vertex>>> vertices = new HashMap<>();
    ...
    public List<List<Vertex>> getAllPaths(Vertex vertex1, Vertex vertex2) {
        ...
        if (vertex1.equals(vertex2)) {
            res.add(vertex1);
            found.add(res);
        } else if (vertices.get(vertex1).stream().map(IEdge::to).anyMatch(v -> v.equals(vertex2))) {
            res.add(vertex1);
            res.add(vertex2);
            found.add(res);
        } else {
    ...
    private List<Vertex> findPath(Vertex vertex1, Vertex target, List<Vertex> visited, List<List<Vertex>> found) {
        List<Vertex> fromEdges = vertices.get(vertex1).stream()
                .map(IEdge::to).filter(v -> !visited.contains(v)).collect(Collectors.toList());
